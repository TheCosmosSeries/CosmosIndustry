...
public class RendererFluidCrafter implements BlockEntityRenderer<BlockEntityFluidCrafter> {
	...
	public RendererFluidCrafter(BlockEntityRendererProvider.Context contextIn) {
		this.context = contextIn;
	}
	
	@Override
	public void render(BlockEntityFluidCrafter blockEntity, float partialTicks, PoseStack poseStackIn, MultiBufferSource buffer, int combinedLightIn, int combinedOverlayIn) {
		Level level = blockEntity.getLevel();
		BlockState state = blockEntity.getBlockState();
		
		if (blockEntity.getCurrentFluidAmount() > 0) {
			if (state.getBlock().equals(IndustryRegistrationManager.BLOCK_FLUID_CRAFTER.get())) {
				Direction dir = state.getValue(BlockFluidCrafter.FACING);
		...
		if (state.getBlock().equals(IndustryRegistrationManager.BLOCK_FLUID_CRAFTER.get())) {
			...
			if (blockEntity.getProcessTime(0) > (blockEntity.getProcessSpeed() - 10)) {
				...
				if (iRecipe != null) {
					ItemStack preOutput = iRecipe.value().getResultItem(level.registryAccess());
					
					if (!preOutput.isEmpty()) {
						ItemStack output = preOutput.isEmpty() ? blockEntity.getItem(3) : preOutput;
