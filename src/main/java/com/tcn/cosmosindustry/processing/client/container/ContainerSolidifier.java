...
public class ContainerSolidifier extends CosmosContainerMenuBlockEntity {
	...
	public ContainerSolidifier(int indexIn, Inventory playerInventoryIn, FriendlyByteBuf extraData) {
		this(indexIn, playerInventoryIn, new SimpleContainer(9), ContainerLevelAccess.NULL, extraData.readBlockPos());
	}

	public ContainerSolidifier(int indexIn, Inventory playerInventoryIn, Container tile, ContainerLevelAccess accessIn, BlockPos posIn) {
		super(IndustryRegistrationManager.CONTAINER_TYPE_SOLIDIFIER.get(), indexIn, playerInventoryIn, accessIn, posIn);
		...
		/**@OutputSlot*/
		this.addSlot(new SlotSolidifier(tile, 3, 62, 40));
		
		/** @Bucket Slots */
		this.addSlot(new SlotBucket(tile, 4, 100, 19));
		this.addSlot(new SlotBucket(tile, 5, 100, 59) {
			@Override
		...
		/** @FluidUpgrade Slots */
		this.addSlot(new SlotUpgrade(tile, 6, 143, 19, IndustryRegistrationManager.UPGRADE_FLUID_USAGE.get()));
		this.addSlot(new SlotUpgrade(tile, 7, 143, 39, IndustryRegistrationManager.UPGRADE_FLUID_CAPACITY.get()));
		this.addSlot(new SlotUpgrade(tile, 8, 143, 59, IndustryRegistrationManager.UPGRADE_FLUID_EFFICIENCY.get()));
	...
	public boolean stillValid(Player playerIn) {
		return stillValid(this.access, playerIn, IndustryRegistrationManager.BLOCK_SOLIDIFIER.get());
	}
	...
	public ItemStack quickMoveStack(Player playerIn, int indexIn) {
		...
		if (flag && slot != null && slot.hasItem()) {
			...
			if (indexIn >= 0 && indexIn < 9) {
				if (!this.moveItemStackTo(itemstack1, 9, this.slots.size() - 9, false)) {
				...
				if (itemstack.getItem() instanceof CosmosItemUpgradeEnergy) {
					if (!this.moveItemStackTo(itemstack1, 0, 2, false)) {
						...
						} else if (!this.moveItemStackTo(itemstack1, 9, this.slots.size() - 9, false)) {
							return ItemStack.EMPTY;
						}
					}
				}
				...
				if (itemstack.getItem() instanceof CosmosItemUpgradeFluid) {
					if (!this.moveItemStackTo(itemstack1, 6, 9, false)) {
						if (indexIn < this.slots.size() - 9) {
							if (!this.moveItemStackTo(itemstack1, this.slots.size() - 9, this.slots.size(), false)) {
								return ItemStack.EMPTY;
							}
