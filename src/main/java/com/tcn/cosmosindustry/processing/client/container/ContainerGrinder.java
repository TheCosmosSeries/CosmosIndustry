...
public class ContainerGrinder extends CosmosContainerMenuBlockEntity {
	...
	public ContainerGrinder(int indexIn, Inventory playerInventoryIn, Container contentsIn, ContainerLevelAccess accessIn, BlockPos posIn) {
		super(ModRegistrationManager.CONTAINER_TYPE_GRINDER.get(), indexIn, playerInventoryIn, accessIn, posIn);
	...
	public boolean stillValid(Player playerIn) {
		return stillValid(this.access, playerIn, ModRegistrationManager.BLOCK_GRINDER.get());
	}
	...
	public ItemStack quickMoveStack(Player playerIn, int indexIn) {
		...
		if (flag && slot != null && slot.hasItem()) {
			...
			if (indexIn >= 0 && indexIn < 6 ) {
				...
				else if (this.getLevel().getRecipeManager().getRecipeFor(ModRecipeManager.RECIPE_TYPE_GRINDING.get(), new SingleRecipeInput(itemstack), getLevel()).isPresent()) {
