...
public class ContainerGrinder extends CosmosContainerMenuBlockEntity {
	...
	public ContainerGrinder(int indexIn, Inventory playerInventoryIn, Container contentsIn, ContainerLevelAccess accessIn, BlockPos posIn) {
		super(IndustryRegistrationManager.CONTAINER_TYPE_GRINDER.get(), indexIn, playerInventoryIn, accessIn, posIn);
		...
		/**@OutputSlot*/
		this.addSlot(new FurnaceResultSlot(player, contentsIn, 1, 124, 29));

		/**@Secondary Output Slot*/
		this.addSlot(new SlotGrinder(contentsIn, 2, 124, 53));
	...
	public boolean stillValid(Player playerIn) {
		return stillValid(this.access, playerIn, IndustryRegistrationManager.BLOCK_GRINDER.get());
	}
	...
	public ItemStack quickMoveStack(Player playerIn, int indexIn) {
		...
		if (flag && slot != null && slot.hasItem()) {
			...
			if (indexIn >= 0 && indexIn < 6 ) {
				...
				else if (this.getLevel().getRecipeManager().getRecipeFor(IndustryRecipeManager.RECIPE_TYPE_GRINDING.get(), new SingleRecipeInput(itemstack), getLevel()).isPresent()) {
