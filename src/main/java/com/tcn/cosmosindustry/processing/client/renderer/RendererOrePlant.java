...
public class RendererOrePlant implements BlockEntityRenderer<BlockEntityOrePlant> {
	private static final ResourceLocation TEXTURE = IndustryReference.Resource.Processing.Render.ORE_PLANT;
	private static final RenderType RENDER_TYPE = RenderType.entityTranslucent(TEXTURE);
	
	private Internals internals;
	private BlockEntityRendererProvider.Context context;

	public RendererOrePlant(BlockEntityRendererProvider.Context contextIn) {
	...
	@Override
	public void render(BlockEntityOrePlant blockEntity, float partialTicks, PoseStack poseStackIn, MultiBufferSource buffer, int combinedLightIn, int combinedOverlayIn) {
...
//				poseStackIn.translate(0,1,0);
		...
		if (state.getBlock().equals(IndustryRegistrationManager.BLOCK_ORE_PLANT.get())) {
			...
			if (dir.equals(Direction.SOUTH)) {
				poseStackIn.mulPose(Axis.YP.rotationDegrees(180));
				poseStackIn.translate(0, 0, -3/16F);
			} else if (dir.equals(Direction.WEST)) {
