...
public class ContainerPeltier extends CosmosContainerMenuBlockEntity {
	
	public ContainerPeltier(int indexIn, Inventory playerInventoryIn, FriendlyByteBuf extraData) {
		this(indexIn, playerInventoryIn, new SimpleContainer(8), ContainerLevelAccess.NULL, extraData.readBlockPos());
	}

	public ContainerPeltier(int indexIn, Inventory playerInventoryIn, Container tile, ContainerLevelAccess accessIn, BlockPos posIn) {
		super(IndustryRegistrationManager.CONTAINER_TYPE_PELTIER.get(), indexIn, playerInventoryIn, accessIn, posIn);

		/** @EnergyUpgrade Slots */
		this.addSlot(new SlotUpgrade(tile, 0, 11, 19, IndustryRegistrationManager.UPGRADE_SPEED.get()));
		this.addSlot(new SlotUpgrade(tile, 1, 11, 39, IndustryRegistrationManager.UPGRADE_CAPACITY.get()));
		this.addSlot(new SlotUpgrade(tile, 2, 11, 59, IndustryRegistrationManager.UPGRADE_EFFICIENCY.get()));
		
		/** @Bucket Slots */
		this.addSlot(new SlotBucket(tile, 3, 83, 19));
		this.addSlot(new SlotBucket(tile, 4, 83, 59) {
			@Override
		...
		/** @FluidUpgrade Slots */
		this.addSlot(new SlotUpgrade(tile, 5, 149, 19, IndustryRegistrationManager.UPGRADE_FLUID_USAGE.get()));
		this.addSlot(new SlotUpgrade(tile, 6, 149, 39, IndustryRegistrationManager.UPGRADE_FLUID_CAPACITY.get()));
		this.addSlot(new SlotUpgrade(tile, 7, 149, 59, IndustryRegistrationManager.UPGRADE_FLUID_EFFICIENCY.get()));
		
		/**@Inventory*/
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 9; x++) {
				this.addSlot(new Slot(playerInventoryIn, 9 + x + y * 9, 8 + x * 18, 107 + y * 18));
			}
		}
		
		/**@Actionbar*/
		for (int x = 0; x < 9; x++) {
			this.addSlot(new Slot(playerInventoryIn, x, 8 + x * 18, 165));
		}
	}
	...
	public boolean stillValid(Player playerIn) {
		return stillValid(this.access, playerIn, IndustryRegistrationManager.BLOCK_PELTIER.get());
	}
		...
		if (flag && slot != null && slot.hasItem()) {
			...
			if (indexIn >= 0 && indexIn <= 7 ) {
				...
				if (itemstack.getItem() instanceof CosmosItemUpgradeFluid) {
					if (!this.moveItemStackTo(itemstack1, 5, 8, false)) {
						if (indexIn < this.slots.size() - 9) {
							if (!this.moveItemStackTo(itemstack1, this.slots.size() - 9, this.slots.size(), false)) {
								return ItemStack.EMPTY;
							}
						} else if (!this.moveItemStackTo(itemstack1, 7, this.slots.size() - 9, false)) {
							return ItemStack.EMPTY;
						}
					}
				}
